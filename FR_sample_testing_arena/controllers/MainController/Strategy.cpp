...
Strategy::Strategy(Maze maze, vector<int> priority_order)
{
    c_maze = maze;
...
int Strategy::find_next_direction(int robot_col, int robot_row, int last_direction, bool has_white)
{
    ...
    if (backtracking_white)
    {
        ...
        if (shortest_path.size() == 0)
        {
            ...
            find_shortest_path(invert_col, invert_row, white_coord[0], white_coord[1], robot);

            for (size_t i = 0; i < shortest_path.size(); i++)
            {
                robot_stack.push_back(get_opposite_dir(shortest_path[i]));
            }
        }
        return selected;
    }
    ...
    {
        if (content_state == WHITE_PATCH && has_white)
        {
        ...
        if (content_state == INVERTED && !has_white && white_locations.size() > 0)
        {
            ...
            find_shortest_path(white_coord[0], white_coord[1], short_col, short_row, robot);
            robot_stack.pop_back();
            for (size_t i = 0; i < shortest_path.size(); i++)
            {
                robot_stack.push_back(get_opposite_dir(shortest_path[i]));
...
int Strategy::find_next_direction_normal(vector<int> visited, vector<int> unvisited)
{
    ...
    if (unvisited.size())
    {
        selected = get_from_priority(unvisited, false);
    }
    else if (visited.size())
    {
        selected = get_from_priority(visited, true);
    }
    return selected;
}
