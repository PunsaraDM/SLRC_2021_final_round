...
{
  motors[2] -> setPosition(target);
  motors[2] -> setVelocity(0.05);
  passive_wait_servo(2,target);
}

void Navigator::arm_vertical_move(double target)
{
  motors[3] -> setPosition(target);
  motors[3] -> setVelocity(0.5);
  passive_wait_servo(3,target);
}

void Navigator::arm_grab_box(double targetLeft, double targetRight)
{
  motors[4] -> setPosition(targetLeft);
  motors[4] -> setVelocity(0.5);
  motors[5] -> setPosition(targetRight);
